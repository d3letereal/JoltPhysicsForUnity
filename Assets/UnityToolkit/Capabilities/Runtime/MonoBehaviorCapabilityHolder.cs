...
namespace Capabilities
{
    public abstract class MonoBehaviorCapabilityHolder<TTag> : MonoBehaviour, ICapabilityHolder<TTag, GameObject>
    {
        ...
        public bool TryGetCapability<TCapability>(out TCapability capability) where TCapability : ICapability
        {
            capability = default;
            foreach (var c in capabilities)
            {
                if (c is TCapability tCapability)
                {
...
#if UNITY_2017_1_OR_NEWER
            UnityEngine.Debug.LogError("TryGetCapability failed for type: " + typeof(TCapability).Name);
#endif
            return false;
        }
        ...
        public bool IsCapabilityActive<TCapability>() where TCapability : ICapability
        {
            ...
            {
                if (capability is TCapability tCap)
                {
                    return tCap.active;
                }
            }
